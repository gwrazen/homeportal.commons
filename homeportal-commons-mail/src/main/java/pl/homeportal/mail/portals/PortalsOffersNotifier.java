 ...
 */
@Stateless
public class PortalsOffersNotifier extends GenericNotifier<PortalsOffersDTO>
{
    private static final Logger LOG = Logger.getLogger(PortalsOffersNotifier.class.getSimpleName());

    private static final String SUBJECT = "email.portals.subject";
    ...
    @Override
    public void notify(PortalsOffersDTO dto)
    {
        try
        {
            VelocityEmail email = new VelocityEmail(EmailType.PORTALS_OFFERS);
            ...
            for ( String address : dto.getEmails() )
            {
                email.addBcc(address);
            }
            LOG.info("Emails sent to: " + dto.getEmails());
    ...
    {
        return configuration.isNotifierEnabled() && configuration.isNotifierPortalsReportEnabled();
    }
